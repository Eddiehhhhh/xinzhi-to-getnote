...
- 在任务中心按标题+日期精确搜索匹配
- 将匹配的任务关联到日记中心
- 如果当天的日记中心页面还不存在，脚本会自动创建后再继续关联
- 额外增加一次凌晨触发，尽量让“刚进入新一天”的任务更早进入当天日记
...
- **日期**：目标日期在任务的日期范围内
- 同名多条时，选择日期最接近的一条
- 如果滴答任务是跨天任务，而任务中心只记录了区间中的某一天，会回退到“唯一同标题 + 日期落在同一区间内”的匹配
...
| `link-tasks-to-diary.yml` | 每天 00:30, 16:00, 23:30 | 关联当天任务到日记 |
